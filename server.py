...
try:
    ...
    print("MediaPipe not available. Hand gesture detection will be disabled.")
    MP_AVAILABLE = False
...
def correct_ocr_errors(text):
    ...
    corrected = re.sub(r'M\s+MH', 'MH', corrected)
    corrected = re.sub(r'([A-Z])\s+([A-Z])', r'\1\2', corrected)
    
    return corrected

def apply_final_corrections(text):
    """Apply final corrections to license plate text"""
    if not text:
        return ""
...
def clean_indian_plate_text(text):
    ...
    cleaned = re.sub(r'[^A-Z0-9\s]', '', str(text).upper().strip())
...
def format_indian_plate(text):
    ...
    if len(text) >= 10:
        return f"{text[:2]} {text[2:4]} {text[4:6]} {text[6:10]}"
    elif len(text) >= 8:
            ...
            for result in results:
                if result.boxes is not None and len(result.boxes) > 0:
                    for box in result.boxes:
                        ...
                        if conf > 0.5 and class_name in vehicle_classes_mapping:
                            vehicle_found = True
                            if conf > best_conf:
                                best_conf = conf
...
def license_plate_detection_phase():
    ...
    # Use relative path - file should be in same directory
    model_path = r"D:\smart_parking project\Smart Parking\best.pt"
    ...
    try:
        ...
        demo_plate = f"{state} {district:02d} {series} {number}"
    ...
    start_time = None
    save_path = "D:\smart_parking project\Smart Parking\detected_plate.jpg"
